...
with_prefix(Prefix) ->
    [MethodName || {MethodName, _} <- proplists:get_value(exports, ?MODULE:module_info()),
...
-define(REQ(Method, Path, Headers),
        httpc:request(Method, {io_lib:format("http://localhost:~B~s", [proplists:get_value(port, Config), Path]), Headers}, [], [])).

-define(REQ_BDY(Method, Path, Headers, Body),
        httpc:request(Method, {io_lib:format("http://localhost:~B~s", [proplists:get_value(port, Config), Path]), Headers, "application/json", Body}, [], [])).
...
e2e_v1_devices_non_json(Config) ->
    ...
    ?CMT("Even if one sets `Content-Type=application/json`, "
         "server is prepared to a malformed body.").

e2e_v1_devices_put_empty(Config) ->
    ?RES(201, _) = ?REQ_BDY(put,
                            ?API_V1_DEVICES ++ ?ID,
                            ?HDS,
                            jiffy:encode(#{})),
...
e2e_v1_devices_put_then_get_all(Config) ->
    ...
    ID = list_to_integer(?ID),
    [#{<<"id">> := ID}] = jiffy:decode(Body, [return_maps]),
    ?CMT("Checks that `GET` returns device in the list preserving last `PUT`.").
...
e2e_v1_devices_put_then_put_then_get(Config) ->
    ...
    ID = list_to_integer(?ID),
    #{<<"id">> := ID,
      <<"state">> := #{<<"battery">> := #{<<"value">> := 98,
                                          <<"unit_of_measurement">> := <<"percent">>,
                                          <<"hardware">> := <<"test_hardware">>}}} = jiffy:decode(Body, [return_maps]),
    ?CMT("Checks that `PUT` indeed updates.").

e2e_v1_devices_delete_non_existing_id(Config) ->
    ?RES(204, _) = ?REQ(delete,
                        ?API_V1_DEVICES ++ ?ID,
                        ?HDS),
...
e2e_v1_devices_put_then_delete_then_get(Config) ->
    ...
    ?RES(204, _) = ?REQ(delete,
                        ?API_V1_DEVICES ++ ?ID,
                        ?HDS),
    ?RES(404, Body) = ?REQ(get, ?API_V1_DEVICES ++ ?ID, ?HDS),
...
e2e_v1_devices_put_null_param_deletes_it(Config) ->
    ...
    ?RES(200, Body) = ?REQ(get,
                           ?API_V1_DEVICES ++ ?ID,
                           ?HDS),
    ID = list_to_integer(?ID),
    #{<<"id">> := ID} = jiffy:decode(Body, [return_maps]),
    ?CMT("`PUT` with `\"parameter_name\": null` stands for the `parameter_name` deletion.").

e2e_v1_devices_put_custom_param_with_custom_uom_and_binary_value(Config) ->
    ?RES(201, _) = ?REQ_BDY(put,
                            ?API_V1_DEVICES ++ ?ID,
                            ?HDS,
                            jiffy:encode(#{<<"state">> => #{<<"custom_param">> => #{<<"value">> => <<"value">>,
    ...
    ID = list_to_integer(?ID),
    #{<<"id">> := ID,
      <<"state">> := #{<<"custom_param">> := #{<<"value">> := <<"value">>,
                                               <<"unit_of_measurement">> := <<"uom">>}}} = jiffy:decode(Body, [return_maps]),
    ?CMT("System supports custom `binary` parameters, units of measurement and values.").

e2e_v1_devices_put_minimal(Config) ->
    ?RES(201, _) = ?REQ_BDY(put,
                            ?API_V1_DEVICES ++ ?ID,
                            ?HDS,
                            jiffy:encode(#{<<"state">> => #{<<"temperature">> => 20,
                                                            <<"humidity">> => 20,
                                                            <<"battery">> => 20}})),
    ?RES(200, Body) = ?REQ(get,
                           ?API_V1_DEVICES ++ ?ID,
    ...
    ID = list_to_integer(?ID),
    #{<<"id">> := ID,
      <<"state">> := #{<<"temperature">> := #{<<"value">> := 20,
                                              <<"unit_of_measurement">> := <<"celsius">>},
                       <<"humidity">> := #{<<"value">> := 20,
                                           <<"unit_of_measurement">> := <<"percent">>},
                       <<"battery">> := #{<<"value">> := 20,
                                          <<"unit_of_measurement">> := <<"percent">>}}} = jiffy:decode(Body, [return_maps]),
    ?CMT("System supports minimalistic representation of the compile-time known params.").
